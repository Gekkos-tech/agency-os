...
description: "Generative Engine Optimization (GEO): make brands and content visible in AI answers — Google AI Overviews, ChatGPT search, Perplexity, Bing Copilot. Analyzes AI crawler accessibility, llms.txt compliance, passage-level citability, brand mention signals, and platform-specific optimization; includes the evidence-led FLOW framework (Find → Leverage → Optimize → Win) with its stage-specific prompt library. Use when the user says 'AI visibility', 'AI Overviews', 'GEO', 'generative engine optimization', 'AEO', 'LLM optimization', 'AI citations', 'ChatGPT search', 'Perplexity', 'llms.txt', or 'how does my brand appear in AI'."
user-invocable: true
argument-hint: "[url]"
license: MIT
metadata:
...
- **FLOW framework** (evidence-led Find → Leverage → Optimize → Win loop, prompt library, CC BY 4.0): [references/flow-framework.md](references/flow-framework.md)
