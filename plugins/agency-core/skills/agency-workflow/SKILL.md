...
description: The agency project router — orchestrates the full client-project lifecycle BRIEF → CONCEPT → DESIGN → BUILD → QA → LAUNCH and tells Claude which specialist skill to load at each phase. Use this skill whenever a client web project starts or advances — trigger on "new project", "neues Projekt", "website for a client", "Website für Kunde/Kunden", "Kundenprojekt starten", "start a client project", "we won a pitch", "relaunch for", or any request to plan, kick off, or continue building a client website, landing page, or web app — even if the user only describes the client and goal ("a website for a café in Cologne"). This skill routes and enforces phase gates; the actual design/build/QA work happens in the specialist skills it delegates to.
---
...
Read the request and project state, then classify:
...
| LAUNCH | [references/launch.md](references/launch.md) | `pre-launch-checklist` (legal/technical launch gate), `seo-audit-suite` or installed SEO skills, `social-formats` (launch assets) |
